...
test('Should throw a noValidatorInfos error', async () => {
  try {
    ...
    await validatorsService.getValidatorUrl('012158b458c095e347653cf4fa625b63e6efa84eae39d08ef8820e9ff7a95b61e0', '2f36a35edcbaabe17aba805e3fae42699a2bb80c2e0c15189756fdc4895356f8', 'casper-test');
    expect(true).toBe(false);
  } catch (e) {
    if (e instanceof NoValidatorInfos) {
      expect(true).toBe(true);
...
test('Should retrieve validator url', async () => {
  const validatorsService = new Validators(casperClient);

  const res = await validatorsService.getValidatorUrl('0124bfdae2ed128fa5e4057bc398e4933329570e47240e57fc92f5611a6178eba5', '2f36a35edcbaabe17aba805e3fae42699a2bb80c2e0c15189756fdc4895356f8', 'casper-test');
  expect(res).toBe('https://node.testnet.casperholders.com');
});

test('Url should not be set', async () => {
  try {
    const validatorsService = new Validators(casperClient);
